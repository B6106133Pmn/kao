...
func (dq *DoctorQuery) QueryDepartment() *DepartmentQuery {
...
func (dq *DoctorQuery) QueryDegree() *DegreeQuery {
...
func (dq *DoctorQuery) QueryNametitle() *NametitleQuery {
	query := &NametitleQuery{config: dq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DoctorQuery) WithDepartment(opts ...func(*DepartmentQuery)) *DoctorQuery {
	query := &DepartmentQuery{config: dq.config}
...
func (dq *DoctorQuery) WithDegree(opts ...func(*DegreeQuery)) *DoctorQuery {
	query := &DegreeQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (dq *DoctorQuery) WithNametitle(opts ...func(*NametitleQuery)) *DoctorQuery {
	query := &NametitleQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withNametitle = query
	return dq
}
...
func (dq *DoctorQuery) sqlAll(ctx context.Context) ([]*Doctor, error) {
	var (
		...
		loadedTypes = [3]bool{
			...
			dq.withDegree != nil,
			dq.withNametitle != nil,
		}
	)
	if dq.withDepartment != nil || dq.withDegree != nil || dq.withNametitle != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := dq.withNametitle; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Doctor)
		for i := range nodes {
